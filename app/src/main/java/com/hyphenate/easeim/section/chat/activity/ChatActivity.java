...
import com.hyphenate.easeim.section.chat.http.httpHelper;
import com.hyphenate.easeim.section.chat.response.UserDetailResp;
import com.hyphenate.easeim.section.chat.utils.SharedPreferencesUtils;
...
public class ChatActivity extends BaseInitActivity implements EaseTitleBar.OnBackPressListener, EaseTitleBar.OnRightClickListener, ChatFragment.OnFragmentInfoListener {
    ...
    private ImageView avatar;  //TODO: 头像
    private TextView rate;  //TODO: 匹配度
    private TextView tag;  //TODO: 标签
    private Button button;  //TODO: 进入主页按钮
    ...
    protected void initView(Bundle savedInstanceState) {
        ...
        rate = findViewById(R.id.chat_rate);
        tag = findViewById(R.id.chat_tag);
        button = findViewById(R.id.chat_btn);
        initChatFragment();
        setTitleBarRight();
        setUserBox();  //TODO: 设置聊天窗顶部的UI
    }
    ...
    private void setUserBox(){
        //TODO: 因为临时赶的，数据请求直接写activity里了
        String token = SharedPreferencesUtils.getToken(getApplicationContext());
        Call<UserDetailResp> call = httpHelper.getGetAPI().getUserDetail(Integer.valueOf(conversationId), token);
        call.enqueue(new Callback<UserDetailResp>() {
            ...
            public void onResponse(Call<UserDetailResp> call, Response<UserDetailResp> response) {
                if(response.body().success){
                    UserDetailResp.UserDetail userDetail = response.body().content;
                    List<UserDetailResp.UserDetail.Tag> tags = userDetail.tags;
                    Glide.with(ChatActivity.this).load(userDetail.avatar).centerCrop().into(avatar);
                    rate.setText("匹配度 "+userDetail.match_rate.toString()+"%");
                    tag.setText("他的引力签：");
                    if(tags!=null){
                        for(UserDetailResp.UserDetail.Tag tag: tags){
                            ChatActivity.this.tag.append(tag.tag_content+"，");
                        }
                    }
                }
                else {
        ...
        button.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                starter.putExtra("u_id", Integer.valueOf(conversationId));
